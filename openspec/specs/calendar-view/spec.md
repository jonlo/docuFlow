...
- **THEN** each label renders as a small coloured chip inline after the event title
...
- **THEN** `onSelectEvent` fires and the `EventDetailModal` opens in read-only mode with the event's fields displayed
