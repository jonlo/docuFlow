...
#### Scenario: Event with labels renders chips
- **WHEN** an event has one or more labels assigned
- **THEN** each label renders as a small coloured chip at the bottom of the event block, showing the label colour and name

#### Scenario: Event without labels renders no chips
- **WHEN** an event has no labels assigned
- **THEN** no label chips are rendered on the event block
...
- **THEN** `onSelectEvent` fires and the `EventFormModal` opens in edit mode with the event's fields pre-populated including its labels
