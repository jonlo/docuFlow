...
The sidebar SHALL display a labelled "Tasks" section with a visual separator above the status accordion. The accordion SHALL show all tasks grouped by status (`in_progress`, waiting, completed). Each row MUST show the task title, a live elapsed-time counter (`HH:MM:SS`) for in-progress tasks, and action icons. The list MUST refresh from `GET /api/tasks` with a 30-second stale time.
